...
export default function MessageInput({ 
  ...
  // Use the search query for mentions if mentionQuery is not null
  const searchResults = useQuery(
    api.users.search, 
    mentionQuery !== null ? { workspaceId: workspaceId as Id<"workspaces">, query: mentionQuery } : "skip"
  );
  ...
  const insertMention = useCallback((user: { name?: string; _id: string }) => {
    ...
    // Store mapping for server-side replacement
    setMentionMap(prev => ({ ...prev, [userName]: user._id }));
  ...
  const handleSend = useCallback(() => {
    ...
    // Replace @name with <@id> before sending
    let processedValue = value;
    Object.entries(mentionMap).forEach(([name, id]) => {
      const escapedName = name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
      const regex = new RegExp(`@${escapedName}\\b`, 'g');
      processedValue = processedValue.replace(regex, `<@${id}>`);
    });

    onSend(processedValue.trimEnd(), selectedImage || undefined);
  ...
  }, [value, selectedImage, onSend, mentionMap]);
  ...
  const handleKeyDown = useCallback(
    ...
    [handleSend, mentionQuery, filteredUsers, isEditing, onCancelEdit, insertMention, selectedIndex],
  );

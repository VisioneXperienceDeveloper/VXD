...
import { Block, RichText } from "../../lib/types";
          ...
          <table className="min-w-full border-collapse border border-neutral-200 dark:border-neutral-700">
            <tbody>
              {block.children?.map((row: Block, rowIndex: number) => (
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                <tr key={row.id} className={rowIndex === 0 && (block as any).table.has_column_header ? "bg-neutral-100 dark:bg-neutral-800" : ""}>
                  {row.type === "table_row" ? row.table_row.cells.map((cell: RichText[], cellIndex: number) => {
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const isHeader = (rowIndex === 0 && (block as any).table.has_column_header) || (cellIndex === 0 && (block as any).table.has_row_header);

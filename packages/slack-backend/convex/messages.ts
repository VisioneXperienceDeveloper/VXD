...
export const send = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const mentions = [...args.body.matchAll(mentionRegex)];
    
    for (const mention of mentions) {
      const mentionedUserId = mention[1] as any;
      
      // Check if mentioned user exists and is a member of the workspace
      const mentionedUser = await ctx.db.get(mentionedUserId);
      if (mentionedUser && "email" in mentionedUser) { // Type guard for user
        const isMember = await ctx.db
          .query("members")
          .withIndex("by_userId_and_workspaceId", (q) => 
            q.eq("userId", mentionedUser._id as any).eq("workspaceId", args.workspaceId)
          )
          .unique();

        if (isMember && mentionedUser._id !== user._id) {
          await ctx.db.insert("notifications", {
            userId: mentionedUser._id as any,
            workspaceId: args.workspaceId,
            channelId: args.channelId,
            messageId,
            type: "mention",
    ...
    if (channel && channel.type === "dm") {
      ...
      if (conversation) {
        const otherUserId = conversation.memberOneId === user._id ? conversation.memberTwoId : conversation.memberOneId;
        
        // Only notify if they weren't already mentioned (to avoid double notification)
...
/**
 * Returns a public URL for an image with optional transformations.
 * Use this for thumbnails or optimized loading.
 */

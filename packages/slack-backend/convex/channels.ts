...
export const get = query({
  ...
  handler: async (ctx, args) => {
    ...
    if (channel.isPrivate) {
      ...
      if (!channelMember) {
        // Fallback for DMs: check if the user is part of the conversation
        if (channel.type === "dm") {
          const conversation = await ctx.db
            .query("conversations")
            .withIndex("by_workspaceId", (q) => q.eq("workspaceId", channel.workspaceId))
            .filter((q) => q.eq(q.field("channelId"), channel._id))
            .unique();
          
          if (conversation && (conversation.memberOneId === user._id || conversation.memberTwoId === user._id)) {
            return channel;
          }
        }
        return null;
